...
fuzzystring provides fast, data.table-backed fuzzy joins for data frames using
string distance metrics from stringdist. Use it when you need to merge rows that
are "close enough" (misspellings, inconsistent labels, user input, or messy IDs).
...
The package includes a set of common misspellings
([Wikipedia list](https://en.wikipedia.org/wiki/Wikipedia:Lists_of_common_misspellings/For_machines)):
...
By default, `fuzzystring_inner_join` uses optimal string alignment (Damerau–Levenshtein distance), and we're setting a maximum distance of 1 for a join. Notice that they've been joined in cases where `misspelling` is close to (but not equal to) `word`:
